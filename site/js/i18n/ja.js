/* CREALIZE i18n — 日本語 */
window.CRZ_I18N = {
  lang: 'ja',
  work: [
    {
      name: 'PurityLens', featured: true, jp: '成分をひと目で', tag: 'AI・ヘルスケア',
      line: '化粧品のラベルを撮影し、成分と判定根拠を確認できます。',
      ph: 'PurityLens — スキャン結果 UI',
      img: '../assets/kv/puritylens.webp',
      url: 'https://puritylens.smartrich.ai/',
      alt: 'PurityLens アプリ — 成分スキャン結果画面',
      stack: ['Flutter', 'AI OCR', 'Serverless API'],
      status: 'shipped',
    },
    {
      name: 'Fudeto', featured: true, jp: '一筆書き', tag: 'パズル・毎日の習慣',
      line: '世界共通の一筆書きパズルを、毎日一問届けます。',
      ph: 'Fudeto — デイリーパズル UI',
      img: '../assets/kv/fudeto.webp',
      url: 'https://fudeto.smartrich.ai/',
      alt: 'Fudeto アプリ — 一筆書きパズル画面',
      stack: ['Flutter', 'Puzzle generator', 'Serverless API'],
      status: 'shipped',
    },
    {
      name: 'Kichitto', featured: true, jp: 'きちっと', tag: 'フィンテック・個人事業主',
      line: '領収書を撮影すると、読み取った内容を自分の Drive と Sheets に保存します。',
      ph: 'Kichitto — 領収書キャプチャ UI',
      img: '../assets/kv/kichitto.webp',
      url: 'https://kichitto.smartrich.ai/',
      alt: 'Kichitto アプリ — 領収書撮影画面',
      stack: ['Flutter', 'Gemini AI', 'Google Workspace API'],
      status: 'shipped',
    },
    {
      name: 'QiFlux', featured: true, jp: '静かな記録', tag: 'ヘルスケア・プライバシー第一',
      line: '記録を端末内に保存する、プライバシー重視の周期トラッカーです。',
      ph: 'QiFlux — トラッキング UI',
      img: '../assets/kv/qiflux.webp',
      url: 'https://qiflux.smartrich.ai/',
      alt: 'QiFlux アプリ — 周期カレンダー画面',
      stack: ['Flutter', 'Riverpod', 'Local-first', 'Serverless API'],
      status: 'shipped',
    },
    {
      name: 'moonpacket', featured: true, jp: '月へ、紅包を', tag: 'Web3・Telegram',
      line: 'Telegram で送れるノンカストディアルのクリプト紅包。USDT・TON・SOL・ETH に対応します。',
      ph: 'moonpacket — 紅包 UI',
      img: '../assets/kv/moonpacket.webp',
      url: 'https://moonpacket.com/',
      alt: 'moonpacket — 濃紺のグラデーション上のブランドマーク',
      stack: ['Telegram Mini App', 'Non-custodial', 'Multi-chain'],
      status: 'shipped',
    },
    {
      name: 'iDokuta', featured: true, jp: '症状を、日本語で', tag: '医療・言語支援',
      line: '英語・中国語・韓国語・ベトナム語で書いた症状を、受付で見せられる日本語に。',
      ph: 'iDokuta — 症状翻訳 UI',
      img: '../assets/kv/idokuta.webp',
      url: 'https://idokuta.smartrich.ai/',
      alt: 'iDokuta アプリ — 症状翻訳画面',
      stack: ['Flutter', 'Realtime translation', '6-locale i18n'],
      status: 'wip',
    },
    {
      name: 'Mairi', featured: true, jp: '毎日のカルテ', tag: 'ヘルスケア・記録',
      line: '血圧・体重・体温・歩数をひとつの記録に。症状チェックと服薬リマインダーも。',
      ph: 'Mairi — デイリーレコード UI',
      img: '../assets/kv/mairi.webp',
      url: 'https://mairi.smartrich.ai/',
      alt: 'Mairi アプリ — ウェルカム画面',
      stack: ['Flutter', 'Next.js', 'LINE integration'],
      status: 'shipped',
    },
    {
      name: 'Tendo', featured: true, jp: '一日一道', tag: 'パズル・毎日の習慣',
      line: 'すべての点を一度ずつ通る、毎日一問の経路パズルです。',
      ph: 'Tendo — デイリーパス UI',
      img: '../assets/kv/tendo.webp',
      url: 'https://tendo.smartrich.ai/',
      alt: 'Tendo ウェブアプリ — ハミルトン路パズル画面',
      stack: ['Vanilla JS', 'Serverless API'],
      status: 'shipped',
    },
    {
      name: 'Rythix 2048', featured: true, jp: '音で解く 2048', tag: 'ゲーム・音楽パズル',
      line: 'プレイに合わせて音楽が生成される、2048型の数字パズルです。',
      ph: 'Rythix 2048 — キービジュアル',
      img: '../assets/kv/rythix2048.webp',
      url: 'https://rythix2048.smartrich.ai/',
      alt: 'Rythix 2048 — リズムのグリッド上で重なるパステルのタイル',
      stack: ['リズムエンジン', '自動生成パズル', 'iOS + Android'],
      status: 'shipped',
    },
    {
      name: 'Meishitto', featured: true, jp: '名刺っと', tag: 'B2B・生産性',
      line: '名刺を端末内で読み取り、連絡先データを自分で管理できます。',
      ph: 'Meishitto — キービジュアル',
      img: '../assets/kv/meishitto.webp',
      url: 'https://meishitto.smartrich.ai/',
      alt: 'Meishitto — 一枚の名刺が構造化されたデータ列へ展開する',
      stack: ['Flutter', '端末内 OCR', 'Riverpod'],
      status: 'shipped',
    },
    {
      name: 'XunNi', featured: true, jp: '尋你', tag: 'AI・占星',
      line: '二人分の命盤をもとに、関係性を読み解くAIサービスです。',
      ph: 'XunNi — キービジュアル',
      img: '../assets/kv/xunni.webp',
      url: 'https://xunni.xyz/',
      alt: 'XunNi — 一本の線で結ばれたふたつの星団',
      stack: ['Capacitor', 'Web + Android', 'AI 鑑定'],
      status: 'shipped',
    },
    {
      name: 'Meguru', featured: true, jp: 'めぐる', tag: 'プラットフォーム・越境EC',
      line: '出品、受注、顧客対応、支払いを一つの運営基盤にまとめます。',
      ph: 'Meguru — キービジュアル',
      img: '../assets/kv/meguru.webp',
      // url: 無 —— 無公開頁（meguru.smartrich.ai 000）（2026-09-04）
      alt: 'Meguru — マゼンタのグラデーション上のブランドマーク',
      stack: ['Cloudflare Workers', 'D1', 'マルチテナント'],
      status: 'shipped',
    },
    {
      name: 'YMY', featured: true, jp: '株式会社YMY商事', tag: 'ブランド · 企業まるごと設計',
      line: 'YMYの企業識別、多言語サイト、事業と流通の仕組みを設計しました。',
      ph: 'YMY — ブランド識別システム',
      img: '../assets/kv/ymy.webp',
      url: 'https://ymytrade.jp/',
      alt: 'YMY コーポレートアイデンティティ',
      stack: ['CI規定', 'ベクター母版', 'Astro · 4言語', 'マスコット体系'],
      status: 'shipped',
    },
    {
      name: 'Kizuki', featured: true, jp: '気付き', tag: 'AI · ソーシャル運用OS',
      line: '商品に関する会話を見つけ、文化に合う返信案を作成します。送信は人が確認して行います。',
      ph: 'Kizuki — 対応インボックス',
      img: '../assets/kv/kizuki.webp',
      url: 'https://kizuki.smartrich.ai/',
      alt: 'Kizuki ソーシャル運用OS',
      stack: ['Cloudflare Workers', 'D1', 'Telegram Bot', '多文化LLM'],
      status: 'wip',
    },
    {
      name: 'dramaflow', featured: true, jp: '短編ドラマ生産ライン', tag: 'AI · 短編ドラマ生産',
      line: '時代設定、衣装、連続性のルールを工程で守る短編ドラマ制作基盤です。',
      ph: 'dramaflow — 制作コンソール',
      img: '../assets/kv/dramaflow.webp',
      // url: 無 —— 內部審核台（2026-09-04）
      alt: 'dramaflow 短編ドラマ生産ライン',
      stack: ['D1', 'Workers', 'OpenArt MCP', 'ショット連鎖'],
      status: 'shipped',
    },
    {
      name: 'Todoke', featured: true, jp: '届け', tag: 'AI · ナレーション動画生成',
      line: '一つの台本から、字幕とナレーション付き動画を3言語で制作します。',
      ph: 'Todoke — パイプライン実行',
      img: '../assets/kv/todoke.webp',
      // url: 無 —— 內部 console（2026-09-04）
      alt: 'Todoke ナレーション動画パイプライン',
      stack: ['video-use', 'HyperFrames', 'TTS・単語字幕', 'YouTube・TikTok'],
      status: 'wip',
    },
  ],
  method: [
    { n: '検証', jp: '', d: 'つくる前に、使いそうな人に話を聞き、思い込みを確かめ、何ができたら成功かを決めます。' },
    { n: '構築', jp: '', d: '経験のある少人数のチームが設計から実装まで行い、最初からそのまま本番に出せる土台で作ります。' },
    { n: 'リリース', jp: '', d: 'まず一つのことがきちんとできる版を出し、使われ方を見て、邪魔になっているところを直します。' },
    { n: '改善', jp: '', d: '実際の使われ方をもとに、速さ、アクセシビリティ、文言、操作感を直し続けます。' },
  ],
  ui: {
    wipBadge: '開発中',
    indexLabel: '全製品目録',
    productsSuffix: ' 製品',
    statusShipped: '● 公開中',
    statusWip: '◌ 開発中',
    filterPlaceholder: '名前・カテゴリ・技術',
    filterHint: 'esc でクリア',
    emptyText: '0 件 — 該当なし · esc でリセット',
    indexHead: ["#", "名前", "和名", "カテゴリ", "状態", "技術"],
    phases: ["想像", "凝縮", "実現"],
    shippedLabel: 'Shipped / 出荷済み',
    wipLabel: 'In development / 開発中',
    formErr: 'オレンジ色の必須項目をご記入ください。',
    formOpening: 'メールアプリを開いています…',
    formOpened: 'メールアプリが開きます — または直接ご連絡ください: support@crealize.llc',
    mailSubject: 'Crealize — お問い合わせ',
    menuLabel: 'メニュー',
    ctaLabel: '{name} を開く',
  },
};
